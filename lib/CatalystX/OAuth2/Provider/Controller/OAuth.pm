...
sub logged_in_required : Chained('base') : PathPart('') : CaptureArgs(0) {
    my ( $self, $ctx ) = @_;
...
sub user_existed_or_authenticated : Private {
    my ( $self, $ctx ) = @_;
    return 1 if $ctx->user_exists();
    return 1
      if $ctx->authenticate(
        {
            username => $ctx->req->param('username') || $ctx->req->param(
                $self->{login_form}->{field_names}->{username}
            ),
            password => $ctx->req->param('password') || $ctx->req->param(
                $self->{login_form}->{field_names}->{password}
            ),
        }
      );
...
sub logged_in_not_required : Chained('base') : PathPart('') : CaptureArgs(0) {
}
...
sub token : Chained('logged_in_not_required') : PathPart('token') #Configurable?
  : Args(0) {
    my ( $self, $ctx ) = @_;
...
sub authorize : Chained('logged_in_required') :
  ...
  : Args(0) {
    ...
    if ( $ctx->req->method eq 'GET' ) {
        $ctx->stash(
            authorize_endpoint => $ctx->uri_for_action( $ctx->action ) );
